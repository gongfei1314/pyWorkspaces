...
修复与改进:
  ...
  [牛熊过滤] 20日均线择时: 现价<MA20*0.98(确认下跌趋势) -> 只卖不买,
             IDLE拒绝进入TRACKING_LOW(禁止抄底), 买入仅允许平仓回补
'''
...
def init(ContextInfo):
    ...
    ContextInfo.MA20_PERIOD      = 20             # [牛熊过滤] 均线周期(20日)
    ContextInfo.MA20_BEAR_RATIO  = 0.98           # [牛熊过滤] 现价<MA20*0.98 -> 熊市只卖不买
    ...
    # ---- [牛熊过滤] 20日均线择时变量 ----
    ContextInfo.ONLY_SELL_MODE  = False          # True=熊市只卖不买(禁止抄底开仓)
    ContextInfo.ma20            = 0.0            # 20日均线值(缓存)
    ContextInfo.ma20_date       = ''             # MA20计算日期(每日仅算一次)
    ContextInfo.bear_block_logged = False        # 熊市拦截打印标志(每日首次)
...
def handlebar(ContextInfo):
    ...
    if ContextInfo.last_trade_date != current_date:
        ...
        ContextInfo.bear_block_logged = False        # [牛熊过滤] 重置熊市拦截打印标志

    ContextInfo.last_bar_close = current_close

    # ============================================================
    # 第零点五步: [牛熊过滤] 20日均线择时 —— 牛熊方向过滤器
    #   MA20 = 截至"昨日"的20根日K收盘均价(不含当日未完成K线, 每日计算一次)
    #   现价 <  MA20*0.98  -> 确认下跌趋势: 只卖不买(禁止抄底开仓)
    #   现价 >= MA20*0.98  -> 恢复正常双向做T
    # ============================================================
    get_ma20(ContextInfo, current_date)
    if ContextInfo.ma20 > 0:
        bear_now = current_close < ContextInfo.ma20 * ContextInfo.MA20_BEAR_RATIO
        if bear_now and not ContextInfo.ONLY_SELL_MODE:
            ContextInfo.ONLY_SELL_MODE = True
            print('[熊市过滤] 现价%.3f < MA20*%.2f(%.3f), 进入只卖不买模式, 禁止抄底' %
                  (current_close, ContextInfo.MA20_BEAR_RATIO, ContextInfo.ma20))
        elif not bear_now and ContextInfo.ONLY_SELL_MODE:
            ContextInfo.ONLY_SELL_MODE = False
            print('[牛市恢复] 现价%.3f >= MA20*%.2f(%.3f), 恢复双向做T' %
                  (current_close, ContextInfo.MA20_BEAR_RATIO, ContextInfo.ma20))
    ...
    if state == STATE_IDLE:
        ...
        if can_trade and change_ratio >= ContextInfo.UP_TRIGGER:
            ...
            if ContextInfo.ONLY_SELL_MODE:
                # [熊市过滤] 下跌触发, 但熊市中禁止抄底 -> 拒绝进入 TRACKING_LOW
                if not ContextInfo.bear_block_logged:
                    print('[熊市过滤] 禁止抄底买入: 跌幅%.2f%%, 现价%.3f < MA20*%.2f(%.3f)' %
                          (change_ratio * 100, current_close, ContextInfo.MA20_BEAR_RATIO,
                           ContextInfo.ma20 * ContextInfo.MA20_BEAR_RATIO))
                    ContextInfo.bear_block_logged = True
            else:
    ...
    # ----------------------------------------------------------
        ...
        if bounce >= ContextInfo.BUY_BOUNCE and ContextInfo.ONLY_SELL_MODE:
            # [熊市过滤] 谷值反弹达标, 但熊市中禁止抄底开仓(规则2.2拦截)
            if not ContextInfo.bear_block_logged:
                print('[熊市过滤] 禁止抄底买入(规则2.2被拦截): 反弹%.2f%%, 现价%.3f < MA20*%.2f(%.3f)' %
                      (bounce * 100, current_close, ContextInfo.MA20_BEAR_RATIO,
                       ContextInfo.ma20 * ContextInfo.MA20_BEAR_RATIO))
                ContextInfo.bear_block_logged = True
        elif bounce >= ContextInfo.BUY_BOUNCE:
            # [Bug1] 仅买入成功才推进状态
...
def get_ma20(ContextInfo, current_date):
    """
    [牛熊过滤] 计算截至 current_date 昨日的20日均线(不含当日未完成K线)。
    每个交易日仅计算一次并缓存(ContextInfo.ma20 / ma20_date);
    依赖 init() 中 download_history_data(..., '1d', ...) 补充的日K历史;
    失败或数据不足返回0.0(过滤当日不生效, 保持双向交易)。
    """
    if ContextInfo.ma20_date == current_date:
        return ContextInfo.ma20                    # 当日已计算, 直接用缓存
    ma = 0.0
    try:
        data = ContextInfo.get_market_data_ex(
            ['close'], [ContextInfo.TARGET_CODE],
        ...
        if df is not None and not df.empty:
            today = current_date.replace('-', '')
            closes = [float(row['close']) for idx, row in df.iterrows() if str(idx)[:8] < today]
            if len(closes) >= ContextInfo.MA20_PERIOD:
                ma = sum(closes[-ContextInfo.MA20_PERIOD:]) / ContextInfo.MA20_PERIOD
                print('[MA20] %s: 截至昨日20日均线=%.3f' % (current_date, ma))
            else:
                print('[警告] 日K仅%d根(需%d), 当日牛熊过滤不生效' % (len(closes), ContextInfo.MA20_PERIOD))
        else:
            print('[警告] 无日K数据, 当日牛熊过滤不生效')
    except Exception as e:
        print('[警告] 计算MA20失败, 当日牛熊过滤不生效:', e)
    ContextInfo.ma20_date = current_date            # 成功与失败都记日期, 避免每根K线重复请求
    ContextInfo.ma20 = ma
    return ma
